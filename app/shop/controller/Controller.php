...
class Controller extends KmdController
{
    ...
    {
        // 商家登录信息
        $this->store = session('kmdshop_store');
        // 当前路由信息
        $this->getRouteinfo();
        //  验证登录状态
        $this->checkLogin();
    ...
    private function saveOptLog(){
        ...
        $model->save([
            ...
            'url' => $this->routeUri,
            'content' => mb_strlen(json_encode($this->request->param(), JSON_UNESCAPED_UNICODE))>65545?'':json_encode($this->request->param(), JSON_UNESCAPED_UNICODE),
