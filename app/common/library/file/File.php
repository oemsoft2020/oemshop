...
class File
{
     ...
     * @return boolean
     */
    public function remove_dir($dir_path, $is_all = true)
    {
        ...
        while (($file = @readdir($handle)) !== FALSE) {
            if ($file != '.' && $file != '..') {
                $dir = $dirName . '/' . $file;
                if ($is_all) {
                    is_dir($dir) ? $this->remove_dir($dir, $is_all) : $this->unlink_file($dir);
                } else {
                    if (is_file($dir)) {
     ...
     * @return string
     */
    function get_next_dir($dir, $num = 1, $more = false)
    {
        $dirArray[] = NULL;
        if (false != ($handle = opendir($dir))) {
            ...
            while (false !== ($file = readdir($handle))) {
                ...
                if ($file != "." && $file != ".." && !strpos($file, ".") && $file != "0") {
                    ...
                    if ($more) $file_more = $this->list_dir_info($dir . "/" . $file, true);
                    $dirArray[$i] = array(
                        "dir" => $file,
                        "dir_more" => $file_more,
                    );
